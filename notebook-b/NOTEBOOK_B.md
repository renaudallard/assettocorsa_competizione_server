...
| `0x37` | 55 | `u8 = 0x37` + 17 × f32 (69 bytes total) | **Periodic weather status broadcast**. Wire layout (verified against an accServer.exe v1.10.2 capture on 2026-04-15): <br><br> `[0]` grip-now (≈ `1 - clouds*0.3`) <br> `[1]` grip-green (constant from `randomizeGreenFlagTriggers`, e.g. 0.97) <br> `[2..4]` reserved zeros <br> `[5]` track wetness (current) <br> `[6]` track wetness (target / mirror of [5]) <br> `[7]` ambient temp °C <br> `[8]` road temp °C <br> `[9]` cloud level (0..1) <br> `[10]` wind direction (degrees, signed; can be negative) <br> `[11]` rain level (0..1) <br> `[12]` wind speed <br> `[13]` dry-line wetness / puddles factor (~0.8 idle) <br> `[14..15]` reserved zeros <br> `[16]` weekend time as f32 seconds <br><br> Emitted from the main server tick tail at a separate cadence (`_DAT_14014bd38`). The body is built by `FUN_1400330e0` which assembles a weather snapshot with `tanhf`-normalized rain / cloud / wet-track values, then calls a virtual `WeatherStatus::serialize` method (`vtable[0x20]`) and finally appends a session-time-delta float. Note: an earlier guess that the WeatherStatus block was 9 floats in `(ambient, road, wind_speed, wind_dir, clouds, rain, wetness, dry_line, puddles)` order is **wrong** — the verified capture shows clouds/rain/wind interleaved as above. |
...
- ~~`WeatherStatus::serialize` body~~ — resolved: vtable[0x20] is `FUN_14011e930` which writes a **9-float WeatherStatus block**, but the wire field order originally inferred from the struct offsets was wrong. A 2026-04-15 capture of accServer.exe v1.10.2 shows the actual broadcast order is: ambient, road, clouds, wind_dir, rain, wind_speed, dry-line/grip-aux, 0, 0 — see §5.6.4 catalog entry for `0x37` for the full 17-float frame layout.
