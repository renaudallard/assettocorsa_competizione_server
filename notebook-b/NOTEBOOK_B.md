...
| `0x2e` | 46 | (varies — see semantics) | **Two distinct uses of this id byte**: <br> **(a) New-client-joined notify** — `u8 + u16 conn_id + u64 timestamp` (11 bytes), pushed by the server to every existing client during a new client's handshake-accept sequence. The sender function **also emits a paired `0x4f` sub-opcode-1 message** (12 bytes: `u8 + u16 carId + u8=1 + u64`) right after the `0x2e`, so the full new-client notification is two messages in sequence. <br> **(b) `ACP_CAR_SYSTEM_UPDATE` relay** — `u8 + u16 carId + u64 system_data + u64 server_timestamp` (19 bytes), broadcast to every other client when one client sends an `ACP_CAR_SYSTEM_UPDATE` (client `0x2e`, see §5.6.1). The two variants share an id byte and are distinguished by their length and call site. A reimplementation joining a second client must emit (a) to the first client at the right moment, and must relay (b) for any system updates from connected cars. |
...
| `0x36` | 54 | `u8 = 0x36` + serialized leaderboard snapshot (variable length, built by `FUN_140034a40`) | **Leaderboard update broadcast** — emitted from the main server tick tail when a "leaderboard dirty" check (`FUN_140115f60`) returns false, indicating the leaderboard has been recomputed. After the broadcast, the function iterates every entry list item and emits a follow-up `0x07` per-car message via the generic serializer (`FUN_14002e080(7, ...)`). Server log after broadcast: `"Updated leaderboard for %d clients (%s-%s %d min)"`. |
| `0x37` | 55 | `u8 = 0x37` + serialized state snapshot (variable length, built by `FUN_1400330e0`) | **Periodic state heartbeat** — emitted from the main server tick tail at a slower cadence than `0x36` (separate timer at `_DAT_14014bd38`). The body is built by a sub-builder that takes the entire server state struct and pulls a snapshot. Distinct from `0x36` (leaderboard) and `0x3e` (general keepalive). |
...
| `0x3e` | 62 | `u8 = 0x3e` + serialized session results (variable length, built by `FUN_1400351f0` from the timing module's `FUN_140131dc0` results getter) | **Session results broadcast** — emitted from the main server tick tail when a session ends. The body carries the full session leaderboard / per-car final results. After the broadcast the function checks the session type and waits `postQualySeconds` (qualy) or `postRaceSeconds` (race) before advancing to the next session. Server log: `"Send session results to %d clients (%d byte)"`. This is the **session result finalization** message clients use to populate the post-session standings screen. |
...
1. **Generic-id builders** (`FUN_14002e080` accepting msg_id as a parameter) emit `0x01` through `0x07` with semantics that the catalog labels by caller context. The exact body bytes for each id depend on the type-erased `void*` argument, which can only be enumerated by decoding the per-type serializers used at each call site.
...
4. The exact body of the `0x40` race weekend reset broadcast — the wire format is built by a virtual `WritableRaceStructure::serialize` method (vtable[0x20]), so the bytes can only be enumerated by either decoding that vtable or capturing a real session.
5. The detailed body order of the `MultiplayerBOPUpdate` `0x53` message — the C++ field set is known (`carId u16` + `restrictor u32` + `ballast i32`) but the on-wire serialization order is determined by the `MultiplayerBOPUpdate::serialize` method.
6. The exact body of the `0x36` leaderboard snapshot built by `FUN_140034a40` and the body of the `0x37` state snapshot built by `FUN_1400330e0`.
