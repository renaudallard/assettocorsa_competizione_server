...
| `0x1e` | 30 | `u8 0x1e` + 63-byte per-car record (64 bytes total) | **Per-car periodic state broadcast — fast-rate, single-record variant.** Pushed from the main server tick (`FUN_14001a170` in accServer) for cars whose dirty flag is set since the last emit. The record body is byte-identical to the `0x39` bundled variant's per-car records. Field map of the 63-byte record (offsets relative to record start, after the msg id): <br><br> `+0x00 u16 car_id` (server source `Car+0x150`) <br> `+0x02 u8 input_state_a` (source `Car+0x2d` — sequence/dirty flag, AC2 compares against last-seen for ordered drop test) <br> `+0x03 s32 server_time_delta_ms` (server-rewritten on relay: `Car+0x3c (last_received_client_ts) − server_basetime_ms`; gates the AC2 1 s look-back drop logic) <br> `+0x07 u16 tick_seq` (source `Car+0x50`) <br> `+0x09 12 B position` (xyz f32 vec3) <br> `+0x15 12 B velocity` (xyz f32 vec3) <br> `+0x21 12 B orient_normal` (heading-basis vec3, used for the reverse-direction check) <br> `+0x2d 4 × u8 wheel_torque[4]` (per-wheel drivetrain torque FL/FR/RL/RR; sourced from physics struct `+0x17f4 + i*0x1040`, log-encoded via `sign(v) × 10^(\|v\| × 127 / log10(maxV))` for high-precision near-zero; verified against drivetrain solver `FUN_1434a9750:278`. NOT driver-input axes — `SteerInput` / `ThrottleInput` / `BrakeInput` / `HandbrakeInput` exist as named fields in the binary but are upstream of physics and are never carried in this packet) <br> `+0x31 u8 scalar_pf6c_p68` (signed-biased `b−127`; sourced from physics `(0xf6c × 0x68)`. JSON serialiser at `1434a1bd0.c:124` emits the same product negated; the JSON dumper writes to anonymous slots so the named purpose isn't recoverable from decomp alone) <br> `+0x32 u8 scalar_doubled_angle` (signed-biased `b−127`, decodes to `(b−127) × 0.5` on the receiver — i.e. an angle in degrees / 2; source `FUN_14347a8a0(param_7) × 2 × 57.29578` where 57.29578 = 180/π. The doubled-rad-to-deg shape is consistent with either yaw rate or steer wheel angle; not pinned without symbols) <br> `+0x33 u16 engine_rpm` (source `Car+0x36`, raw u16 1..65000; verified via `FUN_1434a78b0` rad/s → RPM helper at engine sub-struct `param_7+0x1060`) <br> `+0x35 u8 gear` (source physics `+0x1330`; JSON dumps as `gear − 1` so neutral is `−1`/wraps; raw byte on the wire) <br> `+0x36 u8 scalar_pf64_pct` (normalized 0..1 via `b/255`; physics source `+0xf64 × 255`. JSON serialiser line 131 emits `0xf64 × 100` as a percent, indicating the underlying float is a 0..1 gauge value. Member of the cluster `0xf64 / 0xf68 / 0xf70` — the third (`0xf70`) is dumped as `100 − x×100` "remaining percent", confirming the family is gauge-style — but no offset → name binding exists) <br> `+0x37 u8 scalar_pf68_pct` (same encoding as `+0x36`, source physics `+0xf68 × 255`. Same gauge-style family; same caveat) <br> `+0x38 4 × u8 wheel_input_b[4]` (source physics `+0x1814 + i*0x1040`, encoded `× 25.0` clamped 0..255 → `b/25.0` on receiver, suggested slip-ratio scaled to ~10; FL/FR/RL/RR. Likely drives **tyre smoke / skid-mark / dust particle emitters**) <br> `+0x3c u8 scalar_p1420` (signed-biased `b−127`, source `*(double*)(physics+0x1420)` clamped `[−127, 127]`. JSON line 69 emits the sum `physics+0x1420 + physics+0x1130` to dump slot `+0x28`, suggesting a chassis dynamics field; precise name unknown without symbols) <br> `+0x3d s16 lap_progress_or_rtt` (server-rewritten on relay: `Car+0x1ec` clamped to `[−0x8000, 0x7fff]`; also reused at the top-level wire as `param_6` from caller-supplied RTT/latency. Carries either lap-progress / interpolation-smoothing data on relay or per-conn RTT on the C→S input direction) <br><br>**Source class hierarchy** (recovered via RTTI scan against AC2-Win64-Shipping.exe): the per-tick physics struct serialised in `0x1e` is **`ACCCarSnap`** (vftable VA `0x1445cc330`, typeinfo string `.?AVACCCarSnap@@`), an aggregate of seven ACC-internal sub-snapshot classes:

| Class | vftable VA | Likely role |
|---|---|---|
| `ACCCarSnap` | `0x1445cc330` | top-level per-tick aggregate |
| `ACCRigidBodySnap` | `0x1445cc010` | position / orientation / velocity (the three Vec3s at packet offsets +0x09 / +0x15 / +0x21) |
| `ACCDrivetrainSnap` | `0x1445cc0d8` | wheel torque, gear, RPM (matches `wheel_torque[4]`, `gear`, `engine_rpm`) |
| `ACCDamageSnap` | `0x1445cc4c0` | `scalar_pf68_pct` candidate (CarDamage gauge family) |
| `ACCAISnap` | `0x1445cc1a0` | AI-driver state |
| `ACCTyreStatus` | `0x1445cc268` | `wheel_input_b[4]` candidate (per-wheel slip / dirt) |
| `ACCPhysicsSnap` | `0x1445cc3f8` | `scalar_pf64_pct` and `scalar_p1420` candidates |
| `NetCarStateProvider` | `0x1445c91f0` | Net adapter that wraps `ACCCarSnap` for `0x1e` serialisation |
| `ICarPhysicsStateProvider` | `0x1445c64d8` | Provider interface (base class) |

**Confirmed field-name strings present in AC2-Win64-Shipping.exe** (recovered via `strings` scan with manual section walk):

| String | Likely source class | Likely scalar binding |
|---|---|---|
| `DriveTorque` | `ACCDrivetrainSnap` | `wheel_torque[4]` (verified — single static `lea` xref to the HUD/audio binder at va `0x14344c010`) |
| `Fuel` / `FuelKG` / `fuel` | `ACCPhysicsSnap` or `ACCDamageSnap` | `scalar_pf64_pct` (the 0..1 gauge byte at packet `+0x36`) |
| `CarDamage` | `ACCDamageSnap` | `scalar_pf68_pct` (the 0..1 gauge byte at packet `+0x37`) |
| `LateralG` | `ACCPhysicsSnap` | `scalar_p1420` (signed double at packet `+0x3c`) |
| `yawAngle` | `ACCPhysicsSnap` | `+0x32` doubled-angle byte (the `helper × 2 × rad-to-deg` value) |
| `ThrottleInput` / `BrakeInput` | upstream of the network struct | NOT in this packet (driver-input axes feed physics earlier; only outputs are sent) |

The RTTI hierarchy + string evidence raises confidence on the earlier educated-guess labels significantly. Pinning each scalar to a specific source class member offset still requires either Ghidra symbol recovery on the chassis-physics class members (UE4's stripped FName indirection means `lea`-style xrefs only resolved for `DriveTorque`) or a Steam-Dev-build PDB.

The AC2 client's per-tick JSON dumper (`FUN_1434a1bd0`) writes to **anonymous flat-output slots** rather than calling `FUN_140db3800(L"name", len)` with named UPROPERTY keys. The Niagara / audio binders that DO use named keys (`140dc9320.c`: `wheel_angular_speed`, `slip_ratio`, `tyre_load`, `brake_input`, `nd_slip`) read from a different per-car struct (`param_1+0x2b8`), not from the network-physics struct. Server-side this doesn't matter (the dedicated server is a pure relay; see §14). <br><br> Total = `2+1+4+2+12+12+12+4+1+1+2+1+1+1+4+1+2 = 63 B`. The same id byte is used by client→server `ACP_CAR_UPDATE` (68 B body in that direction, see §5.6.2) — the C→S has the conn_id at offset 4 which the server strips on relay, replacing those 4 bytes with the server-timebase delta and dropping 4 B from the C→S 68 to land at 64 here. |
