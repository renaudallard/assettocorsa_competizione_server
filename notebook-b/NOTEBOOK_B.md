...
| `0x1e` | 30 | **`ACP_CAR_UPDATE`** — the per-tick car state update, sent by each client at simulation tick rate. Total wire size: **68 bytes including the msg id byte**. Exact field-by-field wire format: <br><br> `u8 = 0x1e` (msg id) <br> `u16 source_conn_id` <br> `u16 target_car_id` <br> `u8 gear_or_pit_flag` <br> `u32 client_timestamp_ms` <br> `Vector3 vec_a` (12 bytes — position, x/y/z floats) <br> `Vector3 vec_b` (12 bytes — most likely velocity) <br> `Vector3 vec_c` (12 bytes — most likely angular velocity OR forward direction) <br> `u8 input_a[4]` (4-byte input array #1, likely steering / throttle / brake / clutch as 0-255 normalized) <br> `u8 scalar_32` <br> `u8 scalar_33` <br> `u16 scalar_36` <br> `u8 scalar_2c` <br> `u8 scalar_34` <br> `u8 scalar_35` <br> `u32 scalar_44` (probably another timing field) <br> `u8 input_b[4]` (4-byte input array #2 — possibly traction control / ABS / etc.) <br> `u8 scalar_4c` <br> `i16 scalar_1ec` (sign-extended on receive, likely a signed delta) <br><br> Storage in the server's per-car state: vec_a → +0x8, vec_b → +0x14, vec_c → +0x20 (consecutive 36 bytes); input_a → +0x2e..+0x31; input_b → +0x48..+0x4b. The two 4-byte input arrays are at NON-consecutive storage offsets, so they carry distinct data. <br><br> Server validates that the source connection owns the target car and rejects updates with mismatched ownership (`"Received car update for a different car, connectionId %d. Expected: %d Received: %d"`). It also drops outdated packets where `client_timestamp_ms` is not newer than the last seen timestamp (`"Dropped outdated car_update paket for carId %d, clientTimestamp %d vs lastTimeStamp %d"`). The same id byte `0x1e` is used by server→client periodic state broadcast — see §5.6.4a, the two directions have entirely separate wire formats. |
...
**Still unresolved (down to 2 items):**

1. **Generic-id builders** — `0x01`–`0x07` are built by `FUN_14002e080(msg_id, polymorphic_object, send_target)` which uses vtable[0x58] (`getSerializedSize()`) and vtable[0x68] (`serializeInto()`) on the object. The wire format of each id is type-determined; the seven labels in §5.6.4a are derived from caller context. To get byte-level wire compatibility a reimplementation would need to enumerate which C++ class is passed at each of the ~30 call sites and decode each class's `serialize()` method individually.
2. The **semantic interpretation** of the three Vector3 blocks in `ACP_CAR_UPDATE` (which one is position vs velocity vs angular-velocity vs forward-direction) and which physical inputs the two 4-byte input arrays carry. The wire layout is now exact, but the meaning of each float requires either a packet capture against a real client or much deeper code analysis of how the server consumes those car-state offsets downstream.
