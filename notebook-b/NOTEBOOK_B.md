...
| `Fuel` / `FuelKG` / `fuel` | `0x143c9cdb8` / `0x143c9cdb0` / `0x143c88688` | `fuel_fraction` (`+0x36`) | VERIFIED | pit-stop fuel-loading equation at `FUN_1434a8440:30` uses `physics+0xf64` directly; HUD-data filler `FUN_1434a1bd0:131` writes `physics+0xf64 × 100` to the fuel HUD slot |
| `CarDamage` | `0x143c88c00` | `damage_fraction` (`+0x37`) | VERIFIED | HUD-data filler `FUN_1434a1bd0:132` writes `physics+0xf68 × 100` to the damage HUD slot; sibling field to fuel |
| `LateralG` | (inside `ECarDigitalDisplayType::LateralG`) | `lateralG_or_chassis` (`+0x3c`) | INFERRED-LOW | string exists; encoding (signed double clamped ±127) matches lateral-G semantics; HUD chassis-dynamics accumulator at `FUN_1434a1bd0:69` reads `physics+0x1420 + physics+0x1130`; no direct UE4 name binding |
...
**Why the remaining 2 fields can't be definitively bound:**

`+0x31 (physics+0xf6c × 0x68)` — the product is consumed by HUD/audio code but neither factor has an associated string identifier in the binary. UE4 reflection metadata for the physics-thread class members is not present (the class is a non-reflected internal C++ class, not a UCLASS).

`+0x3c (physics+0x1420)` — the `LateralG` string exists in the binary but appears only in unrelated reflection records (e.g., a broadcasting-overlay struct at offset 0xd8, the MFD pit `FuelData` at offset 0x08, etc.).  None of those reflection records share memory with the physics-thread struct: UE4's reflection mirrors blueprint-visible classes whose data is *written from* the physics thread, but the source class's member offsets aren't reflected.
