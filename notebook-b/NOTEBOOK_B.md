...
| `0x19` | 25 | **Safety-rating contact report** (NOT `ACP_LAP_COMPLETED` — the actual lap-completed event is `0x21`; the `accd/msg.h` constant `ACP_LAP_COMPLETED 0x19` is misnamed and predates the AC2 client RE pass). Body (10 B with msg id): `u8 0x19` + `u16 reporter_car_id` (the player's own car) + `u16 target_car_id` (the other car involved, or `0xffff` for a wall hit) + `i32 timestamp` (server normalises via `FUN_140042030`) + `u8 quality` (signed: `<0` decodes to `-1.0f` = "invalid contact", non-negative → `byte / 10.0f` 0..N rating quality). AC2 client emit sites (verified): `1410be570.c:40,109` (wall-hit / car-to-car contact) and `140e3f9c0.c:960` (SA-contact handler with `"SA contact: %f obwp"` log nearby). Server case `0x19` handler at `1400142f0.c:174-205` builds a queued-broadcast lambda and dispatches a transformed `0x1b` message (see §5.6.4a) to every other client — the broadcast carries the same four fields with field order preserved. The accd reimplementation's `h_lap_completed` reads the right number of bytes but is misnamed and stores the contact `i32 timestamp` as `race->current_lap_ms`, which is incorrect; the actual lap time arrives via `0x21`.|
...
| `0x1b` | 27 | `u8 0x1b` + `u16 reporter_car_id` + `u16 target_car_id` + `i32 timestamp` + `u8 quality` (10 bytes total) | **Safety-rating contact broadcast** — server's queued relay of a client `0x19` contact report. Triggered by any client sending TCP id `0x19` (see §5.6.1). Fields are the same four as the C→S `0x19` body, in identical order: the reporting car's id, the contact-target car id (or `0xffff` for a wall hit), the contact timestamp, and the signed quality byte (`<0` = invalid, otherwise `q/10.0f`). AC2 reader at `143526030.c:793-820` case `0x1b` stores both u16s in a 24-byte `LapBroadcastEntry` at receiver `+0x488`. Server's wire builder at `1400179b0.c` reads `param_1+8 = u16 carA, +0xa = u16 carB, *param_2 = double, +0x18 = float quality` and writes them in that order. An earlier revision of this spec called this row "Lap time broadcast" with `cup_position / track_position` field labels; both name and field interpretation were wrong. |
| `0x1e` | 30 | `u8 0x1e` + 63-byte per-car record (64 bytes total) | **Per-car periodic state broadcast — fast-rate, single-record variant.** Pushed from the main server tick (`FUN_14001a170` in accServer) for cars whose dirty flag is set since the last emit. The record body is byte-identical to the `0x39` bundled variant's per-car records. Field map of the 63-byte record (offsets relative to record start, after the msg id): <br><br> `+0x00 u16 car_id` (server source `Car+0x150`) <br> `+0x02 u8 input_state_a` (source `Car+0x2d` — sequence/dirty flag, AC2 compares against last-seen for ordered drop test) <br> `+0x03 s32 server_time_delta_ms` (server-rewritten on relay: `Car+0x3c (last_received_client_ts) − server_basetime_ms`; gates the AC2 1 s look-back drop logic) <br> `+0x07 u16 tick_seq` (source `Car+0x50`) <br> `+0x09 12 B position` (xyz f32 vec3 — world position; verified empirically X/Z ±500-600m, Y ±5m) <br> `+0x15 12 B orient_normal` (xyz f32 vec3 — Euler angles in radians: yaw, pitch, roll; verified empirically yaw range ±π, pitch/roll ±0.07/±0.10) <br> `+0x21 12 B velocity` (xyz f32 vec3 — m/s; verified empirically max 248 km/h matches GT3 race speeds) <br> `+0x2d 4 × u8 wheel_torque[4]` (per-wheel drivetrain torque FL/FR/RL/RR; sourced from physics struct `+0x17f4 + i*0x1040`, log-encoded via `sign(v) × 10^(\|v\| × 127 / log10(maxV))` for high-precision near-zero; verified against drivetrain solver `FUN_1434a9750:278`. NOT driver-input axes — `SteerInput` / `ThrottleInput` / `BrakeInput` / `HandbrakeInput` exist as named fields in the binary but are upstream of physics and are never carried in this packet) <br> `+0x31 u8 chassis_balance_product` (signed-biased `b−127`; sourced from physics `(0xf6c × 0x68)`.  HUD-data filler `FUN_1434a1bd0:124` emits the same product negated to `HudData+0x74`.  **Empirical behavioural fingerprint** from the 21-min misano pcap (16,735 driving samples filtered to RPM>1000 and speed>5 km/h): symmetric signed quantity centered on 127 (`avg+4.4`, range −127..+127); `|+0x31|` shows **negative correlation with `|wheel_torque|` (Pearson r = −0.43)** and **positive correlation with `|lateralG|` (r = +0.26)`; SD progressively decreases by gear (gear 2 sd 68 → gear 6 sd 24); mean is similar in left turns / right turns / straight (rules out a directional quantity).  Profile fits a chassis-stability / slip-balance metric (candidates: `understeerBalance × tyreLoadFront`, `slipAngleFront × tyreContactFactor`, `frontGrip × tyreLoadFraction`); ruled out: steering wheel angle (would be directional), throttle/brake (no torque correlation), RPM/fuel/damage/speed (no correlation).  Definitive UE4 name binding requires a Steam-Dev-build PDB) <br> `+0x32 u8 yawAngle` [INFERRED-MEDIUM] (signed-biased `b−127`, decodes to `(b−127) × 0.5` on the receiver — angle in degrees / 2; source `FUN_14347a8a0(param_7) × 2 × 57.29578` where 57.29578 = 180/π. The helper computes the yaw-angle delta between two world-direction transforms via vt[+8] / vt[+0x90]. The `yawAngle` string exists in the AC2 binary at VA `0x143c9b118`; the doubled-rad-to-deg encoding matches yaw-angle semantics) <br> `+0x33 u16 engine_rpm` (source `Car+0x36`, raw u16 1..65000; verified via `FUN_1434a78b0` rad/s → RPM helper at engine sub-struct `param_7+0x1060`) <br> `+0x35 u8 gear` (source physics `+0x1330`; JSON dumps as `gear − 1` so neutral is `−1`/wraps; raw byte on the wire.  **Empirically verified** in the 21-minute misano pcap: byte ranges 1..7 with avg 3-4, drops to 1 during the pit-stop window 330-390 s) <br> `+0x36 u8 damage_fraction` [VERIFIED EMPIRICALLY] (normalized 0..1 via `b/255`; physics source `+0xf68 × 255`.  **Empirically verified** by the 21-minute misano pcap: starts at 0.6 (fresh car), accumulates monotonically to 243 (very damaged) over the session as the car takes contact damage.  HUD-data filler `FUN_1434a1bd0:132` writes `0xf68 × 100` (percent) to the damage HUD slot at `HudData+0x10`.  Gauge-selector dispatcher `1434bd310.c` `case 0` reads `physics+0xf68`. **Note**: an earlier revision of this spec had `damage_fraction` and `fuel_fraction` swapped (claimed fuel at `+0x36` and damage at `+0x37`).  The empirical pcap walk corrected this — the wire writer at `FUN_14352f920` emits damage **before** fuel) <br> `+0x37 u8 fuel_fraction` [VERIFIED EMPIRICALLY] (normalized 0..1 via `b/255`; physics source `+0xf64 × 255`.  **Empirically verified** by the 21-minute misano pcap: starts at 246 (≈96% full tank), drains monotonically across the session, drops to ~0 during the pit-stop window 330-390 s, jumps to 234 (refuel) at second 390, then drains again to 9.9 (≈4% near empty) by session end.  Drift is `−236` over 21 minutes with a single +234 jump (the refuel event).  Pit-stop fuel-loading equation at `FUN_1434a8440:30` uses `physics+0xf64` directly as the fuel level; HUD-data filler `FUN_1434a1bd0:131` writes `0xf64 × 100` to the fuel HUD slot at `HudData+0x7c`.  Gauge-selector dispatcher `case 1` reads `physics+0xf64`) <br> `+0x38 4 × u8 wheel_input_b[4]` (source physics `+0x1814 + i*0x1040`, encoded `× 25.0` clamped 0..255 → `b/25.0` on receiver, suggested slip-ratio scaled to ~10; FL/FR/RL/RR. Likely drives **tyre smoke / skid-mark / dust particle emitters**) <br> `+0x3c u8 lateralG` [VERIFIED EMPIRICALLY] (signed-biased `b−127`, source `*(double*)(physics+0x1420)` clamped `[−127, 127]`.  **Empirically verified** by the 21-minute misano pcap: byte ranges 0..254 with avg 127 (signed-bias zero), drift across the session is +1.2 (essentially zero), max-jump 114 (consistent with a hard cornering event).  The signed-balanced swing pattern around 127 with no net drift is the diagnostic signature of a lateral-acceleration field — left-hand corners produce values < 127, right-hand corners produce values > 127, and they balance out over a full lap.  HUD chassis-dynamics accumulator at `FUN_1434a1bd0:69` reads `physics+0x1420 + physics+0x1130` to `HudData+0x28`.  The `LateralG` string exists in the binary as part of `ECarDigitalDisplayType::LateralG`) <br> `+0x3d s16 lap_progress_or_rtt` (server-rewritten on relay: `Car+0x1ec` clamped to `[−0x8000, 0x7fff]`; also reused at the top-level wire as `param_6` from caller-supplied RTT/latency. Carries either lap-progress / interpolation-smoothing data on relay or per-conn RTT on the C→S input direction) <br><br>**Source class hierarchy** (recovered via RTTI scan against AC2-Win64-Shipping.exe): the per-tick physics struct that the `0x1e` wire is built FROM is an **`ICarPhysicsStateProvider`-derived implementation** (base interface RTTI VA `0x1445c64d8`).  This is the live physics-thread struct holding the chassis simulation state.  Other RTTI-recovered classes named `ACC*Snap` (also discovered in the binary) are **NOT** the live struct — they are 0x38-byte protobuf wrapper messages used for a different serialization pathway:

| Class | vftable VA | Role |
|---|---|---|
| `ICarPhysicsStateProvider` | `0x1445c64d8` | live physics-thread interface (base) — the source of `0x1e` wire bytes |
| `NetCarStateProvider` | `0x1445c91f0` | net adapter that pulls from a `ICarPhysicsStateProvider` and feeds the wire writer |
| `ACCCarSnap` | `0x1445cc330` | protobuf wrapper, 0x38 B, NOT the live struct |
| `ACCRigidBodySnap` | `0x1445cc010` | protobuf wrapper, sibling of `ACCCarSnap` |
| `ACCDrivetrainSnap` | `0x1445cc0d8` | protobuf wrapper |
| `ACCDamageSnap` | `0x1445cc4c0` | protobuf wrapper, 0x38 B |
| `ACCAISnap` | `0x1445cc1a0` | protobuf wrapper |
| `ACCTyreStatus` | `0x1445cc268` | protobuf wrapper |
| `ACCPhysicsSnap` | `0x1445cc3f8` | protobuf wrapper, 0x38 B (NOT the live struct, despite the suggestive name) |

The protobuf-wrapper classes' constructors at `FUN_1434be2f0` / `FUN_1434be0c0` allocate exactly 0x38 bytes (5–6 quadwords pointing into separately allocated repeated-field arrays) plus a `*(u32*)(this+0x30) = 1` cached-size field — this is canonical Google protobuf message layout, not a chassis state struct.  The LIVE physics-thread struct (containing offsets `0xf64`, `0xf68`, `0x1420`, etc.) is the implementation behind the `ICarPhysicsStateProvider` interface and has no externally-visible UE4 reflection metadata.

**Confirmed field-name strings present in AC2-Win64-Shipping.exe** (recovered via `strings` scan with manual section walk):

| String | Binary VA | Wire byte | Confidence | Evidence |
|---|---|---|---|---|
| `DriveTorque` | `0x143bc4958` | `wheel_torque[4]` (`+0x2d`) | VERIFIED | static `lea` xref at va `0x14344c010` (HUD/audio binder); drivetrain solver `FUN_1434a9750:278` writes the same offset cluster |
| `Fuel` / `FuelKG` / `fuel` | `0x143c9cdb8` / `0x143c9cdb0` / `0x143c88688` | `fuel_fraction` (`+0x37`) | VERIFIED EMPIRICALLY | misano pcap drift `−236` across 21 min with a refuel `+234` jump at the pit-stop event; pit-stop fuel-loading equation at `FUN_1434a8440:30` uses `physics+0xf64`; HUD-data filler `FUN_1434a1bd0:131` writes `physics+0xf64 × 100` to the fuel HUD slot |
| `CarDamage` | `0x143c88c00` | `damage_fraction` (`+0x36`) | VERIFIED EMPIRICALLY | misano pcap drift `+243` across 21 min (monotonic accumulation as the car takes hits); HUD-data filler `FUN_1434a1bd0:132` writes `physics+0xf68 × 100` to the damage HUD slot |
| `LateralG` | (inside `ECarDigitalDisplayType::LateralG`) | `lateralG` (`+0x3c`) | VERIFIED EMPIRICALLY | misano pcap shows signed-balanced swing around 127 (`b−127` decode) with drift 0.0 and max-jump 114 over 21 min — the diagnostic signature of a lateral-acceleration field that integrates to zero over balanced left/right cornering; HUD chassis-dynamics accumulator at `FUN_1434a1bd0:69` reads `physics+0x1420 + physics+0x1130` |
| `yawAngle` | `0x143c9b118` | `yawAngle` (`+0x32`) | INFERRED-MEDIUM | helper computes a yaw-angle delta between two world-direction transforms (vt[+8]/vt[+0x90]); doubled-rad-to-deg encoding matches; string present but in `WingState` (aero) reflection record at offset 0x3c, not in physics-thread struct |
| `ThrottleInput` / `BrakeInput` | `0x143bc3c30` / `0x143bc3c40` | NOT in this packet | n/a | driver-input axes feed physics earlier; only physics outputs travel on the wire |

The empirical pcap analysis was performed on `notebook-a/captures/kunos_misano_2players_full_session.pcapng` filtered to C→S `0x1e` packets (`udp.length == 76`, dst = server).  The 21-minute session for car_id 1001 yielded 23,111 packets covering practice + qualifying + race + pit stop + race continuation, which is enough to reveal the diagnostic fingerprints (slow monotonic drifts for fuel/damage, signed-balanced swings for lateralG, range-bounded counters for gear).

The gauge-selector dispatcher at `1434bd310.c` / `1434bd640.c` switches on a 9-value internal gauge enum where `case 0 → physics+0xf68`, `case 1 → physics+0xf64`, `case 5 → physics+0xf6c`. This confirms the offset family but the gauge selector is a private enum with no embedded name strings.

**Why the 1 remaining field can't be definitively named:**

`+0x31` — the empirical behavioural fingerprint is now captured (chassis-stability / slip-balance product), but the **specific UE4 source-field name** (whatever the two factors `physics+0xf6c` and `physics+0x68` are called in the AC2 source) requires either Steam-Dev-build PDB symbols or live-runtime debugging.  UE4 reflection metadata for the non-reflected physics class members is absent from the shipping binary.

**Reimplementation impact: zero.** The dedicated server is a pure relay (§14); these names matter only for game-side semantics. For overlay tooling, use the broadcasting-protocol `RealtimeCarUpdate` SDK (§6.3) which exposes the same gauges (Fuel / Damage / LateralG) with documented names. <br><br> Total = `2+1+4+2+12+12+12+4+1+1+2+1+1+1+4+1+2 = 63 B`. The same id byte is used by client→server `ACP_CAR_UPDATE` (68 B body in that direction, see §5.6.2) — the C→S has the conn_id at offset 4 which the server strips on relay, replacing those 4 bytes with the server-timebase delta and dropping 4 B from the C→S 68 to land at 64 here. |
...
| `0x32` | 50 | `u8 = 0x32` + `u16 carId` + `u8 carLocation` (4 bytes) | **`ACP_CAR_LOCATION_UPDATE` relay** — pure event-driven relay of client `0x32` (see §5.6.1). The exe-side handler at `1400142f0.c:672-712` validates `senderCarId == owner_car_id`, stores `carLocation` at runtime `Car+0x153`, then unicasts the rebuilt 4-byte packet to every connection except the sender via `FUN_14001ada0` → `FUN_14004cc50`. The AC2 receiver at `143526030.c:417-481` case `0x32` batches each entry into a `std::vector` at `MPClient+0x3c8` (16-byte slot: u16 carIndex + u8 location + 5 padding + u64 frame_ctx); silent batch (no log line). The vector is consumed downstream by HUD pit indicators, blue-flag logic, and minimap markers. No timer; no welcome inclusion (the value rides inside `CarInfo` at `+0x153` for late-joining clients via `0x23`). The accd reimplementation at `handlers.c:678-851` (`h_car_location_update`) is byte-for-byte correct. |
...
| `0x47` | 71 | `u8 = 0x47` + `u16 carId` + `u8 driver_count` + `driver_count × u8 swap_state` (4 + N bytes) | **Driver swap state broadcast** — server-transformed relay of client `0x47` `ACP_UPDATE_DRIVER_SWAP_STATE`. The body carries the carId and the per-driver swap-state byte. State values (verified against AC2 client UI labels at `notebook-a/decomp/ac2_client_full/140f457f0.c:126-282`, function `UDriverSwapInfoEntry::SetSlot`): <br><br> `0` = (slot omitted from MFD list — used as initial / cleared state) <br> `1` = `OFFLINE` (driver disconnected; rendered grey) <br> `2` = `CONNECTED` (driver online, ready to swap; rendered green) <br> `3` = `REQUESTED` (swap requested, awaiting confirm; rendered red) <br> `4` = `CONFIRMED` (both sides agree, swap pending; rendered saturated red) <br> `5` = `DRIVING` (this driver currently has the wheel) <br><br> Built by `FUN_140011bf0` (accServer) and broadcast via direct TCP send to every connection in the entry list (not via the standard `broadcast_except_one` helper).  See §5.6.4e below for the full state-machine transition graph including event/wire pairings. |
...
A comprehensive sweep plus deep TCP-dispatcher case decoding plus a server-tick-tail walk plus a final inline-write audit has found **32 distinct server → client message IDs** that the stock accServer.exe actually emits.  This list is now considered complete: the audit cross-checked every literal `*(u8*)(...) = 0xNN;` write across all decompiled functions and confirmed that no other byte values appear as msg ids (the few additional bytes found, like `0xc9`, `0xff`, and various offsets, are either lobby internal magic bytes, sentinels, or struct field values rather than transport msg ids).

##### Dead / vestigial AC2 dispatcher slots (never emitted by accServer)

The AC2 client's TCP and UDP dispatchers carry handlers for **six additional message IDs that the stock accServer.exe never emits**. They're listed here so future RE doesn't chase them as protocol gaps:

| ID | Direction | AC2 handler does | Feature path (vestigial) |
|---|---|---|---|
| `0x4c` | TCP | reads `u8` → `MPClient+0x630`; promotes 4→`0x101`, 5→`0x103` | Formation-state delta-update.  The same `+0x630` write also lives in the welcome / `0x4b` event-changed path (`14352a150.c:471-479`); accServer publishes the field exclusively via the welcome trailer and never sends a standalone delta. |
| `0x4d` | TCP | calls `FUN_1435236a0` (`MultiplayerCommunityCompetitionRatingSeries::vftable` ctor with default name `"Standard"`) then `FUN_1434f4e20` (the welcome trailer's RatingSeries reader) | Standalone "send fresh RatingSeries blob" after handshake.  RatingSeries is only ever populated from the welcome trailer; the standalone delta path is unused. |
| `0x50` | TCP | dispatches `vtable[0x28]` of `ksRacing::RaceWeekendForecast` at `MPClient+0x320` — but the vtable slot is `_guard_check_icall`, a 3-byte `ret` no-op stub | Body is `u8 0x50` only (zero payload).  Both ends ship the no-op handler.  Originally a probable RaceWeekendForecast push but the serializer was never written on either end. |
| `0x57` | TCP | empty (`case 0x57: break;`) | Vestigial slot between `0x56` (garage / lap-history) and `0x58` (driver-swap notify).  Likely a stub for a feature subsequently merged into one of those neighbours. |
| `0x5a` | TCP | reads `u8` → vector at `MPClient+0x5f8` (size `+0x600`, capacity `+0x608`); logs `"Received server defined car loc update: %d"` | Server-defined player-car location update, mirror of C→S `0x32` for the driver-swap auxiliary flow.  No reader of the vector exists in the AC2 client, and accServer always uses `0x59` for swap requests instead.  The matching outbound is `14352d970.c` (`AcMultiplayerClient::sendPlayerCarLocationUpdate`). |
| `0x5c` | UDP | parses `(int offset, int length, u16 seq, u8 flag)` and **echoes a `0x5c`-prefixed UDP packet back to the server** with a timestamp from the physics-engine timing context | Connection-quality echo / packet-loss diagnostic, paired with `0x5d` desync warning (`"Bad client connection indication: %d ms desync detected"`).  Dead because accServer never wires the protocol-level echo path; the false-positive matches for `0x5c` in the exe decomp are all ASCII `\` (backslash) bytes inside JSON-string-escape paths and Protobuf wire-tag arithmetic. |

A reimplementation does not need to handle any of these.  They consume zero wire bytes (or specific tagged sequences in the case of `0x5c` UDP) and produce no observable game state.  Future protocol versions could re-activate the formation-state-delta or RatingSeries-delta paths if Kunos chose to ship them, but the AC2 1.10.2 client and accServer 1.10.2 are aligned on the welcome-only convention.
...
#### 5.6.4e Driver-swap state machine

Multi-driver cars (driver swaps in endurance / team-based formats) maintain a per-driver state byte for every driver in the entrylist record.  The state byte values (verified against AC2 client UI rendering at `140f457f0.c:126-282`):

| Byte | UI label | Color | Meaning |
|---|---|---|---|
| 0 | (omitted) | — | Slot not rendered — initial / cleared / fresh-session state |
| 1 | `OFFLINE` | grey (`0xbe212121`) | Driver hasn't joined or has disconnected |
| 2 | `CONNECTED` | green (`0xbe21ff21`) | Driver online, ready to swap |
| 3 | `REQUESTED` | red (`0xe62121ff`) | Swap requested, awaiting confirm |
| 4 | `CONFIRMED` | saturated red (`0xff0000ff`) | Both sides agree, swap pending |
| 5 | `DRIVING` | default | This driver currently has the wheel |
| else | `ERROR` | default | Out-of-range value |

The state machine is driven by these messages:
- C→S `0x47` `ACP_UPDATE_DRIVER_SWAP_STATE` — bulk update (own car: any state; other-owned car: only states 2/3/4 accepted, exe `FUN_140012c30:82-91`)
- C→S `0x4a` `ACP_DRIVER_SWAP_STATE_REQUEST` — single-slot transition; `sub_state ∈ {2, 3, 4}` accepted; sub-state 3 walks every connection sharing the car and demotes 3→2 first
- C→S `0x48` `ACP_EXECUTE_DRIVER_SWAP` — performs the actual swap; requires car in pit (state-byte gate), pit dwell timer ok, and ownership; emits `0x49` reply + `0x58` broadcast + `0x47` cleared-state broadcast on success
- S→C `0x47` — relayed/transformed broadcast of any of the above
- S→C `0x49` — point-to-point reply to the executing client (result code 0 = success, non-zero = error)
- S→C `0x58` `SRV_DRIVER_SWAP_NOTIFY` — broadcast notification of completed swap to other clients
- S→C `0x59` `SRV_DRIVER_HANDOVER_REQ` — sent to the source driver after `&swap <name>` chat command, prompting the AC2 client to fire `executeDriverSwapHandOverCar` which sends `0x48`

Transition table (curr_state, event → next_state, side-effect):

| Curr | Event | Next | Side effect |
|---|---|---|---|
| any | `&swap <name>` chat (own car) | slot[N] = 3 (REQUESTED) | Server emits `0x47` broadcast + `0x59` unicast to source |
| 0/1/2 | C→S `0x47` (own car) | server overwrites slot to whatever the client sent | Server rebroadcasts `0x47` |
| 0/1 | C→S `0x47` (other-owned car) | slot is updated only if value ∈ {2, 3, 4}; else dropped | Server rebroadcasts `0x47` |
| any | C→S `0x4a` sub_state=2, conn_state=v | own driver slot = v | `0x47` rebroadcast |
| any | C→S `0x4a` sub_state=3, conn_state=v | every slot at 3 → 2 first; then own slot = v | `0x47` rebroadcast |
| any | C→S `0x4a` sub_state=4, conn_state=v | own slot = 4 (CONFIRMED) | `0x47` rebroadcast |
| any | C→S `0x4a` sub_state ∉ {2, 3, 4} | rejected; logged | none |
| 4 | C→S `0x48` (in pit, owner, target valid) | `current_driver_index = N`; all slots reset to 0 | `0x49` result=0 unicast + `0x58` broadcast + `0x47` cleared broadcast |
...
| cat | AC2 internal name | Client-side trigger function |
|---|---|---|
| 0 | `Cutting` | `FUN_140e19000:21` (track-cut callback from `updateCarLocation`); also `FUN_140e58c70:420,794` (cutting-while-damaged) |
| 1 | `Collision` | **No client-side detector** (see note below) |
| 2 | `IllegalOvertake` | **No client-side detector** |
| 3 | `PitSpeeding` | `FUN_1410dba70:339` (`PitSpeedingDetector` log at line 320; severity 1/4/6 = DT/SG30/DQ) |
| 4 | `PitEntry` | **No client-side detector** |
| 5 | `PitExit` | **No client-side detector** |
| 6 | `IgnoredMandatoryPit` | end-of-race mandatory-pit miss (`FUN_1410c74b0:1509` — actually emits cat=11, the alias) |
| 7 | `UnsafeRejoin` | **No client-side detector** |
| 8 | `Trolling` | `FUN_14101a5e0:72` (reads existing penalty record; emits when reason byte = 14) |
| 9 | `ReverseInPitlane` | **No client-side detector** |
| 10 | `WrongWay` | `FUN_1410202b0:145,150` (pit-entry check, both branches) and `FUN_1410c74b0:347,353` (over-time-in-pit check) |
| 11 | (alias of 6 — `IgnoredMandatoryPit`) | end-of-race mandatory-pit miss `FUN_1410c74b0:1509` |
| 12 | `ExceededDriverStintLimit` | `FUN_1410c74b0:1407` (negative-delta wrong-way) |
| 13 | `DriverRanNoStint` | **No client-side detector** |
| 14 (0xe) | (out-of-display-enum) | `FUN_1410d1e80:338` — internal "warn-damage" code; server's default branch routes to `RACE_CONTROL` |
| 15 (0xf) | (out-of-display-enum) | `FUN_1410d1e80:338` — internal "lights-off" code; server's default → `RACE_CONTROL` |
| 16 / 17 (0x10 / 0x11) | (out-of-display-enum) | `FUN_140e5ab60:131` (green-flag false-start detector — speeding on start / wrong grid position); server's default → `RACE_CONTROL` |

> **Note on the six "No client-side detector" cats** (1 `Collision`, 2 `IllegalOvertake`, 4 `PitEntry`, 5 `PitExit`, 7 `UnsafeRejoin`, 9 `ReverseInPitlane`, 13 `DriverRanNoStint`): the AC2 client decomp's `1434f2fb0.c` has display-string entries for these cats, and the wire emitter `FUN_14352db40` can carry any cat byte, but no caller of the only emitter wrapper `FUN_140e59bf0` passes those literal cat values anywhere. They are reachable only through the **admin / console penalty injection path** at `FUN_1411a9420` (`"Penalty added via Console for %ls"` log at `140e18ed0.c:38`) — i.e. they are authority-only rulings issued by a server admin, never client-detected events. A reimplementation will see these on the wire only if a remote admin's client injects them via the dev-console; they will not appear from organic gameplay.

> **`accd` `client_category_to_reason` mapping**: was wrong for 13 of the 14 cases pre-v0.3.8 (only cat=0 was correctly mapped — every other client-reported violation got the wrong reason recorded in the per-car PenaltyQueue, propagating to the wire emit and HUD descriptions).  **Fixed in v0.3.8 (commit `c0afc26`)** by rewiring the map to the AC2-client cat enum recovered from `FUN_1434f2fb0`.  Five cats (1 `Collision`, 2 `IllegalOvertake`, 7 `UnsafeRejoin`, 8 `Trolling`, 9 `ReverseInPitlane`) have no equivalent in our `penalty_reason` enum AND no entry in the 0..35 wire mapping, so they fall through to `REASON_RACE_CONTROL` which round-trips as the `*_RaceControl` variants 15..19 on the wire.
...
> **`accd` parity**: was a known gap pre-v0.3.8 (`penalty_total_ms` returned the right total but the per-car PenaltyQueue was not rewritten with the converted TP entries — `results.json` and the post-race HUD showed the original DT/SG kinds rather than the `PostRaceTime` entries Kunos's exe emits).  **Fixed in v0.3.8 (commit `0f5ee2b`)** by adding `penalty_convert_race_end()` in `accd/penalty.c` plus four new enum values `PEN_TP30/40/50/60` and a session-end call site in `accd/tick.c`.  Rewires every unserved DT/SG entry to its post-race-time equivalent at race-only `PHASE_COMPLETED`, gated on `!results_written` so it runs exactly once.
...
```
u16 byte_length
u8[byte_length] UTF-8 data
```

**Verified UTF-8** (NOT UTF-16LE as an earlier revision of this spec claimed): the writer functions internally construct an `std::wstring_convert<std::codecvt_utf8<wchar_t>>` transcoder (verified at `notebook-a/decomp/full/14004b7a0.c:15`).  Each `wchar_t` of the input wide string is transcoded to multi-byte UTF-8 sequences, the resulting byte count is written as the `u16` prefix, then the UTF-8 bytes are `memcpy`'d (`14004d240.c:33` writes the byte count, lines 39-41 emit the bytes).  An ASCII-only string like `"misano"` therefore goes on the wire as `06 00 6d 69 73 61 6e 6f` (8 bytes), NOT `0c 00 6d 00 69 00 73 00 61 00 6e 00 6f 00` (UTF-16LE would be 14 bytes).  Confirmed by `strings(1)` against captured lobby pcaps — literal ASCII strings are visible without interleaved NULs.

Error guard: `"UDPPacket::writeKsonString called with byte length over 65k"` — `byte` length, consistent with the UTF-8 interpretation (a 65-char ASCII string would be 65 bytes UTF-8 / 130 bytes UTF-16, only the former gets close to the cap).
...
- ~~Vector3 semantic interpretation~~ — **fully resolved empirically** (45,523-packet misano pcap walk): `FUN_1400427c0` proves vec_c is the **velocity** vector (`sqrt(x²+y²+z²) × 3.6` km/h check + empirical max 248 km/h matches GT3 race speeds). vec_a is **world position (X, Y, Z)** — empirical X/Z ranges ±500-600 m fit misano track dimensions; Y range ±5 m fits track elevation. vec_b is **Euler-angle orientation** in radians (yaw, pitch, roll) — empirical X range exactly **−π..+π** is the unambiguous yaw signature; Y/Z ranges ±0.07/±0.10 (pitch/roll, naturally small for race cars). Rules out the "forward-direction unit vector" alternative.  Wire byte ordering for both directions is **[position, orient, velocity]** at packet bytes `+0x0a`/`+0x16`/`+0x22` (C→S) or record bytes `+0x09`/`+0x15`/`+0x21` (S→C 0x1e and 0x39 records, verified across 22,436 S→C records).
