...
| `0x28` | 40 | `u8 = 0x28` + `u8 session_index` + 7 × per-slot record + 23-byte tail | **SessionManager state push** — built by `FUN_140033890`, broadcast at ~1 s cadence and as part of the welcome sequence. Each per-slot record is `u8 valid` + (if valid) `f32 ts_in_client_clock`. The 23-byte tail (`FUN_140034f60`) is: `u8 hour_of_day, u8 0, u8 race_flag (1 if session_type==R, else 0), f32 1.0 grip, u16 sched_field (80 for race / 3 otherwise), u32 session_duration_s, u32 session_overtime_s, u8 0, u8 session_type (P=0, Q=4, R=10), f32 1.0`. Verified across 1640 captured frames: tail byte +2 toggles 0/1 strictly on race vs non-race, **not** on `time_multiplier - 1` as an earlier guess assumed. Total size varies (32/40/44/56 bytes) depending on how many of the 7 per-slot timestamps are populated. |
...
| `0x37` | 55 | `u8 = 0x37` + 17 × f32 (69 bytes total) | **Periodic weather status broadcast**. Wire layout derived from `WeatherStatus::serialize` (`FUN_14011e930`) write order `0x28, 0x2c, 0x30, 0x34, 0x3c, 0x38, 0x40, 0x44, 0x48` combined with the field names from the JSON serializer `FUN_140114100` (`ambientTemperature` at +0x28, `roadTemperature` at +0x2c, `windSpeed` at +0x30, `windDirection` at +0x34, `rainLevel` at +0x38, `cloudLevel` at +0x3c): <br><br> `[0]` grip-now (≈ `1 - clouds*0.3`) <br> `[1]` grip-green (constant from `randomizeGreenFlagTriggers`, e.g. 0.97) <br> `[2..4]` reserved zeros <br> `[5]` track wetness (current) <br> `[6]` track wetness (target / mirror of [5]) <br> `[7]` ambient temp °C <br> `[8]` road temp °C <br> `[9]` wind speed <br> `[10]` wind direction (degrees, signed; can be negative) <br> `[11]` cloud level (0..1) <br> `[12]` rain level (0..1) <br> `[13]` WS+0x40 — dry-line wetness / puddles factor (~0.8 idle) <br> `[14]` WS+0x44 — `tanhf`-normalized 0..1, semantics unknown <br> `[15]` WS+0x48 — `tanhf`-normalized 0..1, semantics unknown <br> `[16]` weekend time as f32 seconds <br><br> Emitted from the main server tick tail at a separate cadence (`_DAT_14014bd38`). The body is built by `FUN_1400330e0` which assembles a weather snapshot with `tanhf`-normalized rain / cloud / wet-track values, then calls the virtual `WeatherStatus::serialize` method and finally appends a session-time-delta float. An earlier interpretation of a 2026-04-15 capture placed cloud/rain/wind in slots `[9]/[11]/[12]` — that was wrong: the client's cockpit renderer treats `[12]` as rainLevel, so wind_speed (~0.1) in that slot produced persistent rain on the windshield while the external view happened to read `[11]` and correctly saw cloudLevel = 0 (our rain value). Fixed 2026-04-17. |
...
| `0x4e` | 78 | `u8 = 0x4e` + `u8 conn_count` + per-connection record `{u16 conn_id, u8 flag, i16 ratingA×100, i16 ratingB×100, i16 = -1, i16 = -1, u32 extra_rating, Format-A steam_id}` | **Per-connection rating summary** — emitted periodically from the main server tick tail when a "ratings dirty" flag is set, and also sent to the disconnecting client at disconnect time (with the driver's steam_id in the Format-A field). Verified against an accServer.exe v1.10.2 86-byte capture (1 car, steam_id 17 chars × 4): the two i16 ratings are encoded ×100 (so 20850 = 208.50), followed by **two** i16 = -1 sentinels, **then a u32 extra-rating field** (kunos sends a non-zero value, 0 is accepted), then the Format-A steam_id. |
