...
**Still unresolved (down to 1 item):**

1. **Generic-id builders** — `0x01`–`0x07` are built by `FUN_14002e080(msg_id, polymorphic_object, send_target)` which uses vtable[0x58] (`getSerializedSize()`) and vtable[0x68] (`serializeInto()`) on the object. The wire format of each id is type-determined; the seven labels in §5.6.4a are derived from caller context. To get byte-level wire compatibility a reimplementation would need to enumerate which C++ class is passed at each of the ~30 call sites and decode each class's `serialize()` method individually. This is a tractable but tedious task — each call site is a few minutes of decompilation work.
