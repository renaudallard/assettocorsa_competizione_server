...
**Why the 1 remaining field can't be definitively bound:**

`+0x31 (physics+0xf6c × 0x68)` — the product is consumed by HUD/audio code but neither factor has an associated string identifier in the binary.  UE4 reflection metadata for the physics-thread class members is not present (the class is a non-reflected internal C++ class, not a UCLASS).  The empirical pcap shows this byte averaging `~209` (signed `+82`) with `sd 40` and a wide range — possibly a brake-bias factor, slip-angle, or another product of two normalised physics values.  Closing this name requires a Steam-Dev-build PDB, source code, or live-runtime debugging.
