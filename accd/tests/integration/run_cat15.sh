#!/bin/sh
# cat=15 (LightsOff) tail wire.
# Per reference_kunos_wire_dispatcher.md FUN_1400f03b0:
#   cat=15 kind=5 (TP) -> wire 14 (universal)
#   cat=15 kind=6 (DQ) -> wire 23 (Disqualified_LightsOff)
# The AC2 client main enum (FUN_1434f2fb0) covers 0..13 only, so no
# real client fires cat=15 via 0x41 — kunos's own FUN_140125f50
# callers don't either.  This test exercises the dispatcher path by
# having a bot send 0x41 with cat=15 directly; useful as a regression
# pin for the (renderable) LightsOff wires.
set -e
HERE=$(cd "$(dirname "$0")" && pwd)
cd "$HERE"

echo "combo | accd_tail | kunos_tail | status"
for combo in 15:5:3 15:6:3; do
